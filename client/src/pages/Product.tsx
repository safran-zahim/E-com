    ...
    <div className="min-h-screen bg-app-cream" >
        ...
        <div className="flex gap-8 xl:gap-10">
          ...
          <aside className="hidden lg:block w-64 shrink-0">
            <div className="bg-white rounded-2xl p-4 sticky top-24">
              <FilterPanel category={category}  categories={categoriesData} minPrice={minPrice} maxPrice={maxPrice} organic={organic} hasFilters={hasFilters} clearFilters={clearFilters} updateFilter={updateFilter} />
            </div>
          </aside>
          {/*main content */}
          <main className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6 ">
              ...
              <div className="flex flex-col lg:items-center gap-3">
                {/*Mobile filter */}
                <button onClick={()=> setMobileFilterOpen(true)} className="flex lg:hidden items-center gap-2 px-3 py-2 text-sm bg-white rounded-xl border border-app-border hover:bg-app-cream transition-colors">
                  <SlidersHorizontal className="size-4" /> Filters
                </button>

                {/* Sort */}
                <div className="relative">
                  <select value={sort} onChange={(e)=>updateFilter("sort", e.target.value)} className="appearance-none pl-3 pr-8 py-2 text-sm bg-white rounded-xl border-app-border focus:border-app-green outline-none cursor-pointer" >
          ...
          {/*Pagination*/}
          {totalPages > 1 && (
            <div className="flex-center gsp-2 mt-16">
      ...
      {mobileFilterOpen && (
        <>
          ...
          {/* Bottom Sheet */}
          <div className="fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-2xl max-h-[85vh] flex flex-col animate-slide-in-up">
            
            {/* Header (Pinned) */}
            <div className="p-4 flex justify-between items-center border-b border-app-border shrink-0">
